...
describe('ACL - Accessibility Tests', () => {
  it('should ensure hidden buttons are not tabbable for technician role', () => {
    ...
    const checkTabbableElements = () => {
      ...
      cy.focused().then(($el: any) => {
        // Ensure focused element is not a restricted button
        const testId = $el.attr('data-testid');
        if (testId) {
          expect(testId).to.not.equal('company-create');
          expect(testId).to.not.equal('asset-delete');
          expect(testId).to.not.equal('plan-create');
        }
  ...
  it('should ensure hidden buttons are not tabbable for requester role', () => {
    ...
    const checkTabbableElements = () => {
      ...
      cy.focused().then(($el: any) => {
        const testId = $el.attr('data-testid');
        if (testId) {
          expect(testId).to.not.equal('inventory-create');
          expect(testId).to.not.equal('inventory-edit');
          expect(testId).to.not.equal('inventory-delete');
          expect(testId).to.not.equal('inventory-move');
        }
